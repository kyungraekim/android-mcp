...
class ServiceDiscoveryReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        if (intent.action == ACTION_DISCOVERY_REQUEST) {
            ...
            responseIntent.putExtra(EXTRA_CLASS_NAME, "com.kyungrae.android.timecalculator.TimeCalculatorService")
            responseIntent.putExtra(EXTRA_SERVICE_TYPE, "time") // 서비스 유형
